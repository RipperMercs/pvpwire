...
export function GameSubmitForm() {
    ...
    return (
      <div className="border border-accent/40 bg-accent/5 p-8">
        ...
        <p className="font-serif text-base text-ink/80">
          Submissions are reviewed weekly. Approved entries appear in the games tab without further notice. If you left an email and the editor needs a clarification, you may hear back.
        </p>
        <button
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <Field label="Game name" name="game_name" required placeholder="e.g. Spellbreak" />
      <Field label="Aliases or short names" name="aliases" placeholder="comma separated" />

      <div className="grid sm:grid-cols-2 gap-4">
        <Field label="Developer" name="developer" placeholder="Studio name" />
        <Field label="Publisher" name="publisher" placeholder="Publisher name" />
      </div>

      <div className="grid sm:grid-cols-3 gap-4">
        <Field label="Release year" name="release_year" placeholder="e.g. 2024" />
        <SelectField
          label="Status"
          name="status"
          options={[
        ...
        <SelectField
          label="Category"
          name="category"
          options={GAME_CATEGORIES.map((c) => ({ value: c, label: c }))}
        />
      </div>

      <Field label="Platforms" name="platforms" placeholder="PC, PS5, Xbox, Switch, Mobile" />
      <Field label="PvP modes" name="pvp_modes" placeholder="e.g. team objective, duels, battle royale, faction war" />
      <Field label="Official site URL (optional)" name="official_url" placeholder="https://" />

      <TextareaField
        label="Why this game belongs in the catalog"
        name="why_pvp"
        required
        rows={8}
        placeholder="Describe the PvP loop, the competitive scene, what makes it interesting. Specifics over generalizations: scene size, modes, notable patches, pro-scene presence if any. We will edit for clarity, not for opinion."
      />

      <Field label="Sources or citations (optional)" name="sources" placeholder="Wiki pages, press, store listings, screenshots" />
